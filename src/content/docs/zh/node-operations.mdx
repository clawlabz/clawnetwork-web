...
ClawNetwork 支持两个网络：
- **`--network mainnet`** — 生产网络（推荐大多数用户使用）
- **`--network testnet`** — 开发和测试网络
...
```bash
docker run -d --name claw-node -p 9710:9710 -p 9711:9711 ghcr.io/clawlabz/claw-node:latest start --network mainnet
```
...
# 区块高度
curl -X POST http://localhost:9710 -d '{"jsonrpc":"2.0","id":1,"method":"claw_blockNumber","params":[]}'
```
