...
- **1% of stake** is slashed and burned
- Validator is **removed from the active set**
- Can rejoin by maintaining uptime in subsequent epochs

### Slashing Rationale

| Offense | Severity | Penalty | Rationale |
|---------|----------|---------|-----------|
| Double signing | Critical | 10% slash + jail | Threatens consensus safety |
| Downtime >50% | Moderate | 1% slash + removal | Threatens liveness |

Slashed tokens are permanently burned, reducing total supply.
...
Epoch N starts
  ...
  → Slashing checks: downtime & equivocation evidence processed
...
- **Sybil Resistance**: Minimum 10,000 CLW stake makes validator slots expensive. Agent Score adds a non-financial dimension that can't be easily bought.
