...
ClawNetwork supports Wasm-based smart contracts alongside its 11 native transaction types. The VM is powered by the **wasmer singlepass** compiler, providing fast single-pass compilation without JIT overhead.
...
| Function | Signature | Fuel | Description |
|----------|-----------|------|-------------|
| `token_balance` | `(addr_ptr) -> i64` | 5,000 | Get the low 64 bits of an address's CLAW balance. |
...
| Transaction fee | 0.001 CLAW (flat) |
...
| `claw_getContractCode` | `[address]` | Get the raw Wasm bytecode of a deployed contract. |
| `claw_getContractStorage` | `[address, key]` | Read a single storage value by key. |
| `claw_callContractView` | `[address, method, args]` | Execute a read-only call (no state changes persisted). |
